...
def p1_controls():
    ...
    gamepad.init()
    move = 0
    axis_1 = gamepad.get_axis(1)
    if abs(axis_1) > 0.125:
        move = axis_1 * PLAYER_SPEED
    return move
...
def update():
    global state, game, num_players, a_pressed
    gamepad = pygame.joystick.Joystick(0)
    gamepad.init()

    a_pressed = False
    if gamepad.get_button(0) and not a_pressed:
        a_pressed = True
    a_pressed = gamepad.get_button(0)
    ...
    if state == State.MENU:
        if a_pressed:
            ...
            hat = gamepad.get_hat(0)[1]
            if num_players == 2 and hat == 1:
                game.play_sound("up", menu_sound=True)
                num_players = 1
            elif num_players == 1 and hat == -1:
